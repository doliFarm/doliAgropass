...
/**
 *  \file       doliagropassindicator_document.php
 *  \ingroup    doliagropass
 *  \brief      Tab for documents linked to DoliAgroPassIndicator
 */
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->doliagropass->multidir_output[$object->entity ? $object->entity : $conf->entity]."/doliagropassindicator/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('doliagropass', 'doliagropassindicator', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("DoliAgroPassIndicator")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("DoliAgroPassIndicator"), -1, $object->picto);
...
$param = '&id='.$object->id;
//$relativepathwithnofile='doliagropassindicator/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'doliagropassindicator/'.dol_sanitizeFileName($object->ref).'/';
