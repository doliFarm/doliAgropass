...
/**
 *	\file       htdocs/core/modules/doliagropass/doc/doc_generic_doliagropassindicator_odt.modules.php
 *	\ingroup    doliagropass
 *	\brief      File of class to build ODT documents for doliagropassindicators
 */

dol_include_once('/doliagropass/core/modules/doliagropass/modules_doliagropassindicator.php');
 ...
 */
class doc_generic_doliagropassindicator_odt extends ModelePDFDoliAgroPassIndicator
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=doliagropass_indicator/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="DOLIAGROPASS_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	DoliAgroPassIndicator	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->doliagropass->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new DoliAgroPassIndicator($this->db);
			...
			$dir = $conf->doliagropass->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->doliagropass->dir_temp);
				if (!is_writable($conf->doliagropass->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->doliagropass->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'DOLIAGROPASS_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->doliagropass->dir_temp,
