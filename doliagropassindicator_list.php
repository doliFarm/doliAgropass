...
/**
 * \file       doliagropassindicator_list.php
 * \ingroup    doliagropass
 * \brief      List page for doliagropassindicator
 */
...
include_once __DIR__.'/class/doliagropassindicator.class.php';
...
// Default sort order
if (!$sortfield) {
    if (array_key_exists('code', $object->fields)) {
        $sortfield = "t.code";
    } else {
        reset($object->fields);
...
if (!isModEnabled("doliagropass")) {
    accessforbidden('Module doliagropass not enabled');
}
if (!$permissiontoread) {
    accessforbidden();
}
...
if (empty($reshook)) {
    ...
    $objectclass = 'DoliAgroPassIndicator';
    $objectlabel = 'DoliAgroPassIndicator';
    $uploaddir = $conf->doliagropass->dir_output;
...
$form = new Form($db);
$now = dol_now();
$title = $langs->trans("DoliAgroPassIndicators");
$help_url = '';
$morejs = array();
$morecss = array();
...
// Output page
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-doliagropass page-list bodyforlist');

$arrayofselected = is_array($toselect) ? $toselect : array();

$param = '';
if (!empty($mode)) $param .= '&mode='.urlencode($mode);
if (!empty($contextpage) && $contextpage != getDolDefaultContextPage(__FILE__)) $param .= '&contextpage='.urlencode($contextpage);
...
foreach ($search as $key => $val) {
    if (is_array($search[$key])) {
        foreach ($search[$key] as $skey) {
            if ($skey != '') $param .= '&search_'.$key.'[]='.urlencode($skey);
        }
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/doliagropass/doliagropassindicator_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

$topicmail = "SendDoliAgroPassIndicatorRef";
$modelmail = "doliagropassindicator";
$objecttmp = new DoliAgroPassIndicator($db);
...
foreach ($object->fields as $key => $val) {
    ...
    if ($key == 'calculation_method' || $key == 'input_type') $cssforfield .= ' center'; 
    ...
    if ($mode == 'kanban' || $mode == 'kanbangroupby') {
        ...
        foreach ($object->fields as $key => $val) {
            ...
            if (!empty($arrayfields['t.'.$key]['checked'])) {
                ...
                // [MOD] CODE IS THE LINK
                elseif ($key == 'code') {
                    print '<a href="'.dol_buildpath('/doliagropass/doliagropassindicator_card.php', 1).'?id='.$object->id.'">';
                    // Show Icon + Code
                    print img_object('', $object->picto).' '.$object->code;
                    print '</a>';
                }
                else {
                    if ($val['type'] == 'html') print '<div class="small lineheightsmall twolinesmax-normallineheight">';
                    
                    $finalValue = (string) $object->$key;

                    // [MOD] Icon Only + Tooltip for Calculation Method
                    if ($key == 'calculation_method') {
                        $translatedText = $langs->trans($finalValue);
                        if (strpos($finalValue, 'AUTO') === 0) {
                            print '<span class="classfortooltip" title="'.$translatedText.'"><span class="fa fa-magic text-info"></span></span>';
                        } elseif (strpos($finalValue, 'MANUAL') === 0) {
                            print '<span class="classfortooltip" title="'.$translatedText.'"><span class="fa fa-user-edit text-warning"></span></span>';
                        } else {
                            print $translatedText;
                        }
                    }
                    // [MOD] Icon Only + Tooltip for Input Type
                    elseif ($key == 'input_type') {
                        $translatedText = $langs->trans($finalValue);
                        if (strpos($finalValue, 'SCALE') !== false || strpos($finalValue, 'SELECT') !== false) {
                            print '<span class="classfortooltip" title="'.$translatedText.'"><span class="fa fa-list-ol text-primary"></span></span>';
                        } elseif ($finalValue == 'BOOLEAN') {
                            print '<span class="classfortooltip" title="'.$translatedText.'"><span class="fa fa-toggle-on"></span></span>';
                        } elseif ($finalValue == 'NUMBER') {
                            print '<span class="classfortooltip" title="'.$translatedText.'"><span class="fa fa-hashtag"></span></span>';
                        } else {
                            print $translatedText;
                        }
                    }
                    // [MOD] Standard Translation for other fields
                    elseif (in_array($key, array('label', 'description', 'category_group'))) {
                        print $object->showOutputField($val, $key, $langs->trans($finalValue), '');
                    }
                    else {
                        print $object->showOutputField($val, $key, $finalValue, '');
                    }
