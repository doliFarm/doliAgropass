...
/**
 *  \file       doliagropassaudit_note.php
 *  \ingroup    doliagropass
 *  \brief      Tab for notes on DoliAgroPassAudit
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->doliagropass->multidir_output[empty($object->entity) ? $conf->entity : $object->entity]."/".$object->id;
}
...
if ($enablepermissioncheck) {
	...
	$permissionnote = $user->hasRight('doliagropass', 'doliagropassaudit', 'write'); // Used by the include of actions_setnotes.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
	$permissionnote = 1;
...
$title = $langs->trans('DoliAgroPassAudit').' - '.$langs->trans("Notes");
//$title = $object->ref." - ".$langs->trans("Notes");
$help_url = '';
//$help_url='EN:Customers_Orders|FR:Commandes_Clients|ES:Pedidos de clientes';

llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-doliagropass page-card_notes');
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("DoliAgroPassAudit"), -1, $object->picto);
