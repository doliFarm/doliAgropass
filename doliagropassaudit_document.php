...
/**
 *  \file       doliagropassaudit_document.php
 *  \ingroup    doliagropass
 *  \brief      Tab for documents linked to DoliAgroPassAudit
 */
...
$langs->loadLangs(array("doliagropass@doliagropass", "companies", "other", "mails"));
...
$upload_dir = null;
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->doliagropass->multidir_output[$object->entity ? $object->entity : $conf->entity]."/doliagropassaudit/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('doliagropass', 'doliagropassaudit', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("DoliAgroPassAudit")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-doliagropass page-card_document');

// Show tabs
$head = doliagropassauditPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("DoliAgroPassAudit"), -1, $object->picto);
...
$modulepart = 'doliagropass';
$param = '&id='.$object->id;
//$relativepathwithnofile='doliagropassaudit/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'doliagropassaudit/'.dol_sanitizeFileName($object->ref).'/';
