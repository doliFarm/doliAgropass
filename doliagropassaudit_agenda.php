...
/**
 *  \file       doliagropassaudit_agenda.php
 *  \ingroup    doliagropass
 *  \brief      Tab of events on DoliAgroPassAudit
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->doliagropass->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	$title = $langs->trans("DoliAgroPassAudit")." - ".$langs->trans('Agenda');
	//$title = $object->ref." - ".$langs->trans("Agenda");
	$help_url = 'EN:Module_Agenda_En|DE:Modul_Terminplanung';

	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-doliagropass page-card_agenda');
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("DoliAgroPassAudit"), -1, $object->picto);
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		//$cachekey = 'count_events_doliagropassaudit_'.$object->id;
