...
/**
 *  \file       doliagropassindicator_note.php
 *  \ingroup    doliagropass
 *  \brief      Tab for notes on DoliAgroPassIndicator
 */
...
if ($enablepermissioncheck) {
	...
	$permissionnote = $user->hasRight('doliagropass', 'doliagropassindicator', 'write'); // Used by the include of actions_setnotes.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
	$permissionnote = 1;
...
$title = $langs->trans('DoliAgroPassIndicator').' - '.$langs->trans("Notes");
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("DoliAgroPassIndicator"), -1, $object->picto);
