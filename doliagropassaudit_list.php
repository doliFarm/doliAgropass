...
/**
 *   	\file       doliagropassaudit_list.php
 *		\ingroup    doliagropass
 *		\brief      List page for doliagropassaudit
 */
...
include_once __DIR__.'/class/doliagropassaudit.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('doliagropass', 'doliagropassaudit', 'read');
	$permissiontoadd = $user->hasRight('doliagropass', 'doliagropassaudit', 'write');
	$permissiontodelete = $user->hasRight('doliagropass', 'doliagropassaudit', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DoliAgroPassAudit';
	$objectlabel = 'DoliAgroPassAudit';
...
$title = $langs->trans("DoliAgroPassAudits");
//$help_url = "EN:Module_DoliAgroPassAudit|FR:Module_DoliAgroPassAudit_FR|ES:Módulo_DoliAgroPassAudit";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/doliagropass/doliagropassaudit_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-doliagropass page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/doliagropass/doliagropassaudit_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDoliAgroPassAuditRef";
$modelmail = "doliagropassaudit";
$objecttmp = new DoliAgroPassAudit($db);
